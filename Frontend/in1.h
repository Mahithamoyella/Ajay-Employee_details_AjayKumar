...
<body>
    ...
    <script>
        async function fetchEmployees() {
            try {
                const response = await fetch('http://44.202.114.151:3065/api/employees');
        ...
        async function fetchAllUsers() {
            try {
                const response = await fetch('http://44.202.114.151:3065/api/all-users');
        ...
        function showUserProfile(user) {
            ...
            if (user.profile_image) {
                ...
                imageUrl = `http://44.202.114.151:3065${normalizedPath}`;
                const img = new Image();
                img.onerror = () => {
        ...
        document.getElementById('addEmployeeForm').addEventListener('submit', async function(event) {
            ...
            showConfirmDialog('Confirm Add Employee', 'Are you sure you want to add this employee?', async () => {
                try {
                    const response = await fetch('http://44.202.114.151:3065/api/add-employee', {
                        method: 'POST',
                        body: formData
                    });
        ...
        function displayEmployeeCard(employee) {
            ...
            const imageUrl = employee.profile_image 
                ? `http://44.202.114.151:3065/${employee.profile_image}` 
        ...
        async function deleteEmployee(employeeId) {
            ...
            showConfirmDialog('Confirm Delete', `Are you sure you want to delete ${employee ? employee.name : 'this employee'}?`, async () => {
                try {
                    const response = await fetch(`http://44.202.114.151:3065/api/delete-employee/${employeeId}`, {
                        method: 'DELETE'
                    });
                    const result = await response.json();
        ...
        function editEmployee(employeeId) {
            ...
            if (employee.profile_image) {
                preview.src = `http://44.202.114.151:3065/${employee.profile_image}`;
                preview.style.display = 'block';
            } else {
                preview.src = '';
                preview.style.display = 'none';
